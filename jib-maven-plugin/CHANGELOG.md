...
### Added
- Additionally reads credentials from `.docker/.dockerconfigjson` and legacy Docker config (`.docker/.dockercfg`). Also searches for `$HOME/.docker` in (addition to current `System.get("user.home")`). This may help retrieve credentials, for example, on Kubernetes. (#[2260](https://github.com/GoogleContainerTools/jib/issues/2260))
