...
- `jib.container.entrypoint = ['INHERIT']` allows inheriting `ENTRYPOINT` and `CMD` from the base image. While inheriting `ENTRYPOINT`, you also override `CMD` using `jib.container.args`.
