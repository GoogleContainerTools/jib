...
- Support for local base images using a URL scheme in `jib.from.image`. Use the format `docker://name-of-image` to use a base image stored in the local Docker daemon, or `tar:///path/to/file.tar` to use a base image tarball stored on the filesystem (may be absolute or relative) ([#1468](https://github.com/GoogleContainerTools/jib/issues/1468), [#1905](https://github.com/GoogleContainerTools/jib/issues/1905))
