...
`outputFiles` | [`outputFiles`](#outputfiles-closure) | See [`outputFiles`](#outputfiles-closure) | Configures the locations of additional build artifacts generated by Jib.
...
<a name="outputfiles-closure"></a>`outoutFiles` is a closure with the following properties:

Property | Type | Default | Description
--- | --- | --- | ---
`tar` | `File` | `(project-dir)/build/jib-image.tar` | The path of the tarball generated by `jib:buildTar`.
`digest` | `File` | `(project-dir)/build/jib-image.digest` | The path of the image digest written out during the build.
`id` | `File` | `(project-dir)/build/jib-image.id` | The path of the image id written out during the build.
