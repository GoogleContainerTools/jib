...
* You did not configure auth information in the default places where Jib searches (see also [Authentication Methods](https://github.com/GoogleContainerTools/jib/blob/master/jib-maven-plugin/README.md#authentication-methods) )
   - a credential file (as generated by `docker login` or `podman login`) in:
     - `$XDG_RUNTIME_DIR/containers/auth.json` or `$XDG_CONFIG_HOME/containers/auth.json`
     ...
     - You can look for a log message similar `Using credentials from Docker config (/home/myuser/.docker/config.json)` to figure out which credential file was used.
