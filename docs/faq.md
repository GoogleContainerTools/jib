...
* You did not configure auth information in the default places where Jib searches.
   ...
   - `$HOME/.docker/config.json` may also contain short lived authorizations in the `auth` block that may have expired. In the case of Google Container Registry, if you had previously used `gcloud docker` to configure these authorizations, you should remove them by editing your `config.json` and deleting lines from the `auths` section associated with `gcr.io` (for example: `"https://asia.gcr.io"`). You can then run `gcloud auth configure-docker` to correctly configure the `credHelpers` block for more robust interactions with gcr.
