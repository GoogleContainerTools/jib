...
* You previously used the deprecated `gcloud docker` to configure the `auths` block in `config.json`. Those credentials are short lived and can expire. They can be removed by editing  your `config.json` and removing `auths` associated with `gcr.io` (for example: `"https://asia.gcr.io"`). You can then run `gcloud auth configure-docker` to correctly configure the `credHelpers` block for interactions with the Google Container Registry.
