...
* You previous used the deprecated `gcloud docker` to configure the `auths` block in `config.json`. Those credentials expire and need to be removed. Edit your `config.json` and remove `auths` associated with `gcr.io` (for example: `"https://asia.gcr.io"`). You can run `gcloud auth configure-docker` to correctly configure the `credHelpers` block for interactions with Google Container Registry.
