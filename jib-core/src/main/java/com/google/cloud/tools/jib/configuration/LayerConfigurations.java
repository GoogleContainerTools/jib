...
public class LayerConfigurations {
  ...
  /**
   * Stores the {@link LayerConfiguration}s and allows for access by label. The labels should be
   * unique, but if not, the last layer with a certain label will be referenced by that label.
   *
  ...
  public LayerConfigurations(List<LayerConfiguration> layerConfigurations) {
    ...
    ImmutableMap.Builder<String, LayerConfiguration> layerConfigurationMapBuilder =
        ImmutableMap.builderWithExpectedSize(layerConfigurations.size());
