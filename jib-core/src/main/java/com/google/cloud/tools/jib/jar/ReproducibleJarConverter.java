 ...
 * https://github.com/Zlika/reproducible-build-maven-plugin and https://github.com/gradle/gradle
 * (ZipCopyAction.java)
 */
public class ReproducibleJarConverter {
  ...
  @VisibleForTesting
  static final ImmutableList<String> MANIFEST_ATTRIBUTES_TO_STRIP =
      ImmutableList.of(
          "Bnd-LastModified",
          "Build-Jdk",
          "Build-Date",
  ...
  public boolean isSigned() throws IOException {
    ZipFile zip = new ZipFile(jar.toFile());
    List<ZipArchiveEntry> entries = Collections.list(zip.getEntries());
    Optional<ZipArchiveEntry> searchResult =
        entries
            .stream()
            .filter(zipArchiveEntry -> zipArchiveEntry.getName().matches("META-INF/.*\\.SF"))
            .findAny();
  ...
  private void handleManifest(
      ZipArchiveEntry entry, ZipArchiveEntry srcEntry, ZipFile zip, ZipArchiveOutputStream out)
      throws IOException {

    Manifest manifest = new Manifest(zip.getInputStream(srcEntry));

    // process manifest -- strip the fields
    MANIFEST_ATTRIBUTES_TO_STRIP.forEach(
        key -> {
  ...
  private void standardizeAttributes(ZipArchiveEntry entry) {
    ...
    if (entry.isDirectory()) {
      entry.setUnixMode(
          (0b0100 << 12) + FilePermissions.DEFAULT_FOLDER_PERMISSIONS.getPermissionBits());
    } else {
      entry.setUnixMode(
          (0b1000 << 12) + FilePermissions.DEFAULT_FILE_PERMISSIONS.getPermissionBits());
    }
