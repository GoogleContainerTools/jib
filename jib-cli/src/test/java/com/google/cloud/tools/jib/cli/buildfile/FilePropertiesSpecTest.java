...
public class FilePropertiesSpecTest {
  ...
  public static void createObjectMapper() {
    filePropertiesSpecMapper = new ObjectMapper(new YAMLFactory()).configure(DeserializationFeature.FAIL_ON_IGNORED_PROPERTIES, true);
  }
  ...
  public void testFilePropertiesSpec_full() throws JsonProcessingException {
    ...
    Assert.assertEquals(FilePermissions.fromOctalString("755"), parsed.getDirectoryPermissions().get());
  ...
  public void testFilePropertiesSpec_badFilePermissions() throws JsonProcessingException {
    ...
    try {
      ...
      Assert.assertEquals("octalPermissions must be a 3-digit octal number (000-777)", ex.getCause().getMessage());
    }
  }
  ...
  public void testFilePropertiesSpec_badTimestamp() throws JsonProcessingException {
    ...
    try {
      ...
      Assert.assertEquals("timestamp must be a number of milliseconds since epoch or an ISO 8601 formatted date", ex.getCause().getMessage());
    }
  }
}
