...
public class WarFiles {
     ...
     */
    public static JibContainerBuilder toJibContainerBuilder(ArtifactProcessor processor,  CommonCliOptions commonCliOptions, ConsoleLogger logger) throws IOException, InvalidImageReferenceException {
        JibContainerBuilder containerBuilder = Jib.from("jetty");
        List<FileEntriesLayer> layers = processor.createLayers();
        List<String> entrypoint = processor.computeEntrypoint(new ArrayList<>());
