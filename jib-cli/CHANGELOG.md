...
- Changed the default base image of the Jib CLI `jar` command from the `adoptopenjdk` images to the `eclipse-temurin` images on Docker Hub. Note that Temurin by Adoptium is the new name of AdoptOpenJDK. ([#xyz](https://github.com/GoogleContainerTools/jib/pull/xyz))
